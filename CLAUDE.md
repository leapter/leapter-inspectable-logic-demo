...
**Always run `pnpm dev` from the repo root — never from `web/`.** The root
script launches two processes side-by-side: a tiny `leapter convert --watch`
watcher that recompiles `.vts` blueprints to JSON when they change, and the
Next.js dev server on :4000. The web app executes blueprints in-browser via
`@leapter/runtime-browser`, so there is no separate runtime server.
`web/package.json`'s `dev` is just `next dev` — running it alone skips the
blueprint converter, so the app falls back to whatever JSON happens to be
on disk. If you `cd`'d into a subdirectory for another step, `cd` back to
the repo root before starting the dev server.

Wait for the Next.js server to be ready ("✓ Ready in …"), then tell the user:
...
**If port 4000 is already in use** (e.g. another copy of the starter is
...
(port 4000 is busy). Would you like me to stop the other one so this one can
start? If you want to keep both running, let me know and I'll set this one up
on a different port.
```

Only shut down the other process after the user confirms. To find it:
`lsof -i :4000` (macOS) or `netstat -ano | findstr :4000` (Windows).
...
.leapter-tools/cli/leapter convert --project ./leapter --out web/src/leapter-blueprints
```
...
| **Compiled blueprints** | `web/src/leapter-blueprints/` | LogicFlowModel JSON, generated by `leapter convert` (gitignored) |
| **UI** | `web/src/modules/` | Form, schema, page component for the app |
| **Unified runtime client** | `web/src/lib/runtime.ts` | Branches between in-browser (local mode) and Server Action (remote mode) |
| **API bridge (remote only)** | `web/src/app/actions/` | Server Actions that call the hosted Leapter runtime |
...
| **Browser runtime** | `packages/runtime-browser/` | `@leapter/runtime-browser` - bundled `LogicRuntime` for in-page execution |
| **Remote runtime client** | `packages/leapter-client/` | `@leapter/client` — HTTP client to the hosted Leapter runtime |

**Rules:**
- NEVER put business logic in TypeScript/React code — it goes in Leapter blueprints
- NEVER put UI code in Leapter blueprints
- ALL blueprint calls go through `web/src/lib/runtime.ts` - never call `@leapter/runtime-browser` or the Server Actions directly from a page
- API key is ONLY used server-side via `@leapter/client` (remote mode)
...
### 4. Test locally
```bash
# From repo root — starts the blueprint converter (watch mode) and Next.js
pnpm dev
...
### 5. Deploy
```bash
# Validate locally, then deploy the web app. Push blueprints only for hosted runtime/API use.
pnpm validate
cd web && vercel deploy --prod
# optional hosted runtime/API path:
# pnpm push
```
...
│   ├── src/lib/runtime.ts             ← Unified blueprint client (local or remote)
│   ├── src/leapter-blueprints/        ← Compiled blueprint JSON (gitignored)
│   ├── src/app/actions/blueprint.ts   ← Server Actions for remote-mode HTTP
│   └── src/components/ui/             ← shadcn/ui (pre-installed)
├── packages/
│   ├── leapter-client/                ← @leapter/client — HTTP client (remote mode only)
│   └── runtime-browser/               ← @leapter/runtime-browser - bundled local runtime
...
### Calling a blueprint from the page

Always go through `web/src/lib/runtime.ts`. It branches between local mode
(in-browser via `@leapter/runtime-browser`) and remote mode (HTTP via the
Server Actions in `web/src/app/actions/blueprint.ts`). Pages should not
import either path directly.

```tsx
// web/src/modules/pizza-pricing/page.tsx
"use client";

import { runBlueprint } from "@/lib/runtime";
import { projectConfig as project } from "@/lib/project";

const response = await runBlueprint(
  { projectSlug: project.slug, projectId: project.projectId },
  project.blueprintSlug,
  values, // { pizzaSize, toppings, crustType, dayOfWeek }
);
...
The `traceData` field is the same shape Glass Mode replays - works
