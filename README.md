...
Or start with the agent workflow:
...
This starter uses a local Leapter runtime during development:
...
For this starter, no Leapter SaaS account is required to validate and execute the included local Blueprint. Pushing Blueprints to Leapter Lab is optional and requires login.
...
Start the local runtime and app:
...
Run the pizza pricing logic directly:
...
`pnpm push` requires Leapter login. Local validation and execution do not.
...
| Leapter / Veritas | Logic is text in Git, visual in the editor, executable at runtime, and traceable after each run |

## Current State

This is an early starter, built to test the workflow and invite feedback.
...
- the best UX for editing specifications and diagrams together
